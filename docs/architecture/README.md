...
- **DECIDED:** Authentication is a fully independent NestJS system for all roles, local JWT, no Supabase Auth even during Frontend Migration ([ADR-005](decisions/ADR-005-authentication.md) Option C). Locked roles: **`admin`** (buyer / owner), **`staff`** (same permissions as admin for now), **`customer`** (shop member login). NestJS natively handles admin temp-passwords/resets and customer OTP.
