...
- Migration risk stays bounded: no parallel authz model in Next.js. **Identity and permission checks for sessions are NestJS** from Product MVP (Ship 1) ([ADR-005](ADR-005-authentication.md) Option C). Existing RLS/storage rules still apply to leftover client data paths until Frontend Migration Phase 2 ([ADR-006](ADR-006-server-boundaries.md)). Do not treat Supabase Auth as the IdP.
