...
- [x] Nest JWT HTTP-only cookie / SSR session: **ACCEPTED RISK** — prove Next proxy + RSC validation; remains **BLOCKED** until PASSED. The `@supabase/ssr` spike ([spikes/auth-ssr-spike.md](spikes/auth-ssr-spike.md)) is **superseded** and must not be used as the remaining proof. CSRF mechanism is **DECIDED** (D-37 / [ADR-017](decisions/ADR-017-csrf-cookie-mutations.md)): Origin/Host + `SameSite=Lax`; include foreign-origin rejection in the D-28 proof.
