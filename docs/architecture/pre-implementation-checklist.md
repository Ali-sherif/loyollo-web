...
- [x] Supabase server-session / HTTP-only cookie approach: **ACCEPTED RISK** ΓÇö prove after migration start (foundation / server-infra / auth slices); remains **BLOCKED** until PASSED ([spikes/auth-ssr-spike.md](spikes/auth-ssr-spike.md); needs service-role bootstrap or confirmed `SPIKE_TEST_*` credentials)
