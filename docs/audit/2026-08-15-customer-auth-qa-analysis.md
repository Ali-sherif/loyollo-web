...
**Amendment 2026-08-18 (auth stack):** Merchant and customer identity are **NestJS Auth API** only ([ADR-005](../architecture/decisions/ADR-005-authentication.md) Option C). Supabase is database storage + RLS on leftover client paths — not an IdP. Legacy shipped code may still reference withdrawn Supabase Auth; document as migration debt.
...
**Auth stack (DECIDED — [ADR-005](../architecture/decisions/ADR-005-authentication.md) Option C):** **NestJS** handles authentication and JWT issuance for all roles. **Supabase** is **not** an IdP — it remains a direct data-fetching layer (PostgreSQL storage + RLS policies) on leftover client paths until Frontend Migration Phase 2 ([ADR-011](../architecture/decisions/ADR-011-rls-storage-strategy.md)). Legacy shipped code may still call withdrawn Supabase Auth APIs; treat those as migration debt, not the target contract.

| Plane | Identity | Password | Status |
|---|---|---|---|
| **Merchant** (`admin`/`staff`) | **NestJS Auth API** email/password (local JWT) | Yes (+ TOTP MFA when in scope) | **Partially shipped** — sign-up/sign-in/verify/reset UI exists (legacy may still hit Supabase Auth until cutover); `staff` invite, `role`, `account_status` do **not exist in schema** |
| **Customer** (shop loyalty member) | **NestJS Auth API** phone-based OTP (SMS/WhatsApp) | **Never** — passwordless by lock ([11-auth §Shop-customer register](../frontend/11-authentication-migration.md#shop-customer-register-and-login-decided)) | **DECIDED, not shipped.** Today the customer has no login at all — only `POST /api/join/enroll` with OTP, no session, no wallet, no return login |
...
| L-08 | Refresh/expiry: silent token refresh keeps user logged in past the JWT's short expiry | Merchant | **Target:** NestJS Auth API refresh token rotation. **Legacy (shipped):** Supabase localStorage auto-refresh — withdrawn. Next.js HTTP-only cookie / SSR equivalent is unproven (`D-28`, see §3) |
...
| **Medium** | Leading/trailing/internal whitespace in email, e.g. `" user@x.com "`, `"us er@x.com"` | Verify normalization before the NestJS Auth API sign-up/sign-in call and before OTP phone match (`otp_verifications` keys on exact phone). |
...
| **Low** | Case sensitivity in email (`User@Example.com` vs `user@example.com`) creating duplicate accounts | NestJS Auth API must normalize email (typically lower-case) before persistence; confirm this is not bypassable via direct API calls. |
...
| **High** | Expired/invalid NestJS-issued email confirmation or password-reset link | Merchant plane — verify link expiry behavior and that resubmission works without ambiguous state. |
...
| **High** | Login with wrong password for a real vs. non-existent email — must return the same generic message/timing | NestJS Auth API must enforce timing-safe, generic responses; verify once Next.js proxy owns the request path. |
...
| — | **No account-lockout / failed-login-attempt policy** for the merchant password login, anywhere in `docs/`. | Currently untestable as pass/fail; the NestJS Auth API must define and enforce backoff/lockout — do not rely on withdrawn Supabase Auth defaults. |
