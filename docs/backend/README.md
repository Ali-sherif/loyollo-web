...
| Doc | Purpose |
|-----|---------|
| [data-contract.md](data-contract.md) | Target schema (independent programs, `customer_program_memberships` two counters, `reward_snapshot` / versions, visit_events, orders + invoice/currency, **`profiles.role` + `profiles.account_status`** (S-01), **`profiles.currency` locked at onboarding**, **Business Type + Industry closed lists (UX-21)**, OTP PM-06, vouchers, referrals, catalog redeem + PM-04, mutation guards, PM-08 tier functions, write rules, glossary). Product model: [program-model.md](../product/program-model.md) · [ADR-016](../architecture/decisions/ADR-016-independent-programs.md) |
