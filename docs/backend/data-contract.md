...
### `profiles` — role and account status (S-01; G-33, G-34, G-36)

**Status:** DECIDED, not shipped. Closes the runtime security conflict where product rules require role- and status-aware access but the schema has neither column ([S-01](../audit/2026-08-14-security-ui-product-audit.md#s-01--authz-is-login-only-critical--g-33-g-34-g-36)).

Backend-owned Prisma migration ([ADR-014](../architecture/decisions/ADR-014-product-data-ownership.md)). **Do not** add this migration in the frontend repo.

| Column | Type | Null | Default | Notes |
|--------|------|------|---------|-------|
| `role` | enum | no | `'customer'` | **`admin`** \| **`staff`** \| **`customer`**. Locked stored names — [locked role matrix](../frontend/11-authentication-migration.md#locked-role-matrix). Do **not** use `merchant_admin`, `staff_cashier`, or `purchaser`. Writers set explicitly on sign-up / invite / customer register. |
| `account_status` | enum | no | `'active'` | **`active`** \| **`inactive`** \| **`pending`**. Account-level gate — distinct from `customers.status`, program status, and campaign status. |

**`role` writers:**

| Event | `role` written |
|-------|----------------|
| Merchant self sign-up (buys Loyollo) | `admin` |
| Admin creates teammate ([G-34](../frontend/gaps-and-solutions.md#g-34--admin-cannot-create-adminstaff-with-emailed-temp-password)) | `admin` \| `staff` per form |
| Customer register / enroll identity ([G-33](../frontend/gaps-and-solutions.md#g-33--shop-customers-have-no-registerlogin-kpis-rely-on-owner-typed-rows)) | `customer` |

**`account_status` semantics:**

| Value | Meaning | `/app` access | Customer login |
|-------|---------|---------------|------------------|
| `active` | Normal account | Allowed for `admin` / `staff` | Allowed for `customer` |
| `inactive` | Admin deactivated ([G-36](../frontend/gaps-and-solutions.md#g-36--no-admin-account-list-or-activeinactive-for-staffcustomer)) | **Denied** — even after password reset | **Denied** — even after OTP |
| `pending` | Teammate invited, temp password not yet changed ([G-34](../frontend/gaps-and-solutions.md#g-34--admin-cannot-create-adminstaff-with-emailed-temp-password)) | **Denied** until first-login password change | N/A for `customer` |

**Enforcement (both layers required):**

1. **NestJS** — every merchant-only handler rejects `role === 'customer'` or `account_status !== 'active'` with **403** `FORBIDDEN_ROLE` or **403** `ACCOUNT_NOT_ACTIVE`. JWT claims must carry `role` and `account_status` ([api-contract](api-contract.md#auth--session)).
2. **Next.js** — middleware / proxy on `/app/*` applies the same rule before RSC or client bundles load ([11-authentication-migration.md](../frontend/11-authentication-migration.md#route-guards-app)). Client checks are UX only.

**Indexes:** `(role)` for account-list filters ([G-36](../frontend/gaps-and-solutions.md#g-36--no-admin-account-list-or-activeinactive-for-staffcustomer)); `(account_status)` for active/inactive filters.

### `profiles` — merchant display currency (UX-23 / DG-09)

The Shop’s **display currency** is a merchant-level property on `profiles`, not per-program and not per-order.

| Column | Type | Null | Notes |
|--------|------|------|-------|
| `currency` | text | no* | ISO 4217 code (e.g. `USD`, `EGP`, `SAR`, `AED`). *Required once onboarding completes. **Display metadata only** — symbol/label for merchant UI; does **not** convert stored `*_cents`, points, or vouchers. |

**Lifecycle (DECIDED):**

1. **Set once at onboarding** — the merchant selects currency during account setup (onboarding step 1 today: “Average cheque currency”). On save, write **`profiles.currency`** (consolidate from legacy `cheque_currency`; do not maintain two competing columns long term).
2. **Immutable after setup** — once `onboarding_completed = true`, `profiles.currency` is **read-only** in Settings General. The UI shows the code disabled; PATCH/UPDATE from Settings must omit `currency` (backend should reject changes with **400** `CURRENCY_LOCKED` if a client tries anyway).
3. **No retroactive conversion** — changing display currency (if ever allowed by support) must **not** rewrite historical `orders.amount_cents`, `points_ledger`, or `rewards.cost_cents`. Each monetary row snapshots its own `currency_code` at write time.

**Allowed writers:**

| Writer | When |
|--------|------|
| Onboarding upsert | First time only, before or when `onboarding_completed` flips to `true` |
| Support / migration (later) | Explicit admin tool only — out of merchant self-serve |

**Frontend consumers (target):** Dashboard revenue stat cards, Analytics revenue widgets, Settings General (read-only display), and any shared `formatMoney(cents, currency)` helper must read `profiles.currency` for **aggregate shop-level** amounts. Per-row displays (orders list, POS receipt) use the row’s `currency_code` snapshot when present, else fall back to `profiles.currency`. **Never** hardcode `$`, `USD`, or `en-US` in merchant-facing widgets.

**Distinct from:**

- **`orders.currency_code` / `points_ledger.currency_code`** — immutable snapshot at transaction time ([orders](#orders), [points_ledger](#points_ledger)).
- **Program earn rate** — currency-to-points conversion is a **program rule** ([program-model.md](../product/program-model.md#4-customer-membership-and-wallet)); not controlled by changing `profiles.currency`.
...
11. **Account status (DECIDED, not shipped):** persisted on `profiles.account_status` as `active` \| `inactive` \| `pending`. `admin` sets `staff` and `customer` to `active` \| `inactive`. `pending` is set when a teammate is created with a temp password ([G-34](../frontend/gaps-and-solutions.md#g-34--admin-cannot-create-adminstaff-with-emailed-temp-password)) and cleared to `active` after first-login password change. Any value other than `active` blocks the account’s surface (`/app` for merchant roles; customer login for `customer`). Distinct from `customers.status` (which includes `deleted` for soft-delete). **One page, two tabs:** Team (`admin` + `staff`) and Customers (`customer`). Filters: role (Team tab), email, name, phone; active/inactive on both. [11-authentication-migration.md](../frontend/11-authentication-migration.md#account-active--inactive-decided). **Never HARD DELETE** a customer; GDPR erasure sets `status=deleted`, nulls PII, retains `phone_hash`.
...
| **Staff** (`staff`) | Works for that shop. Uses `/app`. **For now: same permissions as `admin`.** Subtypes and a later permission split are not locked. Not a loyalty customer. | `profiles.role = 'staff'`; scoped to Shop via `owner_id`. [G-32](../frontend/gaps-and-solutions.md#g-32--contact--admin-plan-limits-unused) |
| **Customer** (`customer`) | Shops at that business. Customer register/login (will exist), not `/app`. Data in `customers`. Status/tier are not roles. KPIs calculated from stored activity. Soft-delete only. | `profiles.role = 'customer'`; linked to `customers` row. [G-33](../frontend/gaps-and-solutions.md#g-33--shop-customers-have-no-registerlogin-kpis-rely-on-owner-typed-rows) |
...
| **Account status** | Can this user use their surface: `profiles.account_status` = `active` (نشط) \| `inactive` (غير نشط) \| `pending` (teammate first-login). **Not** member `customers.status`, not program status. `admin` toggles `active`/`inactive` for `staff` and `customer` on **one page, two tabs** (Team / Customers). | [profiles role and account status](#profiles--role-and-account-status-s-01-g-33-g-34-g-36) · [11-authentication-migration.md](../frontend/11-authentication-migration.md#account-active--inactive-decided) |
