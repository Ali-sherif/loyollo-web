...
**Available vs reserved (DECIDED):** pending catalog redemptions reserve `points_cost` in that program. `Available = Total − Reserved`. Do not spend a lot after `expires_at` (how reservation interacts with lot expiry is **pending Product Owner** — [redemption §14 item 14](../product/reward-redemption-flow.md#14-pending-owner-decisions-do-not-implement-yet)).
...
| `status` | text | no | catalog redeem Phase 1: `pending` \| `completed` \| `rejected`. Optional later (not Phase 1): `cancelled` \| `expired` \| `reversed`. Visit **earn** rows may still use `earned` until an explicit redeem |
...
| `customer_rewards` pending/completed + reserved `points_cost` | Catalog redeem reserve / approve | G-20 |
...
8. **Authz:** owner-scoped to `loyalty_program_id` / `owner_id`. Service-role only in workers and public enroll ([ADR-006](../architecture/decisions/ADR-006-server-boundaries.md)). Merchant roles: **`admin`** (buyer) and **`staff`** (**same permissions as `admin` for now**). Do not use stored name `purchaser`. Catalog redemption approve/reject is **Shop-level**: `staff.branch.shop_id === redemption.program.shop_id`. Any authorized Staff from any Branch of that Shop may process. Staff from another Shop must not. Never authorize on the redemption ID alone. Phase 1: any existing Staff or Admin role may perform Redemption operations; do not add extra role restrictions unless decided later.
...
15. **Catalog redemption lifecycle (DECIDED Phase 1, not shipped):** create is `pending` + reserve `points_cost`; create is idempotent (same operation returns the same row; do not reserve twice). Approve is one transaction (`PENDING → COMPLETED` + consume reserved); already `completed` → no-op / same result (no second deduct). Reject releases. Combined pending cost cannot exceed available. Cross-program spend is forbidden; the row stays on `loyalty_program_id` even if the customer later uses another program. Reward eligibility / expiry is evaluated at create; later reward `expires_at` must not auto-invalidate a pending row. Earn and redeem share one concurrency model. Idempotency key / unique business reference on create and approve (and on earn — write rule 16). **Not Phase 1:** refund / reverse. **Pending Product Owner:** price change / reward disable-delete / program disable / reserved-lot expiry while PENDING. [reward-redemption-flow.md](../product/reward-redemption-flow.md) · [api-contract](api-contract.md#catalog-redemption-lifecycle-decided-not-shipped).
