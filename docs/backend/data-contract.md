...
## Shop capability model (DECIDED, not shipped)

Canonical product: [program-model.md](../product/program-model.md). This section is the schema implication. **Do not** add these as migrations in this frontend repo.

**Shop identity:** there is **no** `shops` table today. The Shop is `owner_id` / `profiles`. Do not invent a `shops` table from this contract unless the backend program later adds one with the same product meaning.

| Object | Today | Target |
| ------ | ----- | ------ |
| `loyalty_programs` | `UNIQUE (owner_id)` — one row total | **Capability configs.** `UNIQUE (owner_id, program_type)` — at most one `points`, one `visit`, one `tier`. Optional `status` `draft` \| `active` \| `disabled` per row. |
| `customers` | One row per `loyalty_program_id` | **One membership per Shop.** Balances `points`, `visits`, `tier` / `tier_id` live on that row. Unique phone/email (when present) is per Shop, not per capability. |
| `visit_events`, `orders`, `points_ledger`, `referrals`, `otp_verifications`, `vouchers`, `customer_rewards`, `rewards`, `qr_page_settings`, `referral_settings`, `insight_actions`, `campaigns` | FK `loyalty_program_id` meaning “this program” | **Shop-scoped.** Target FK is the Shop (`owner_id` / shop identity). Existing column name `loyalty_program_id` may remain as a transitional alias that resolves to the Shop (or to the matching capability row when the table is capability-specific, e.g. `loyalty_program_tiers`). Backend owns the rename. |
| `loyalty_program_tiers` | FK to a `tier` program row | FK to the Shop’s **single** Tier capability row |
| Rewards catalog | Per program | **Shop-wide.** `point_cost` redeem requires Points enabled. |

Invalid: two rows with the same `(owner_id, program_type)`. Valid: Points + Visit + Tier (three rows). Join/check-in requires **at least one** `active` capability.

**Open:** whether `assign_customer_tier` reads spendable `customers.points` / `visits` or a lifetime cumulative metric — [program-model.md](../product/program-model.md#open--tier-metric-source). **Open:** referral `points` kind when Points is disabled.
...
Lots are **Shop-scoped**. Shop A’s 100 points and Shop B’s 200 points are different memberships and must never be summed for spend or for the customer wallet.

**Available vs reserved (DECIDED):** pending catalog redemptions reserve `points_cost` in that Shop **at Redeem time** (not at staff-scan time). `Available = Total − Reserved`. Concurrent Redeems check Available, not Total. Do not spend a lot after `expires_at` (how reservation interacts with **lot** expiry is **pending Product Owner** — [redemption §14 item 14](../product/reward-redemption-flow.md#14-pending-owner-decisions-do-not-implement-yet)). QR / pending TTL is 10 minutes and is independent ([§6b](../product/reward-redemption-flow.md#6b-expiry-handling-scheduled-job)).
...
| `referral_code` | text | no | stable per member; unique per Shop; generated at enroll. Share URL is this Shop’s join URL `?ref={referral_code}` and personal QR encode that URL |
...
Unique: `(shop identity, referral_code)` — one code per Shop membership. Share URL is this Shop’s join URL `?ref={referral_code}`.
...
| Column | Type | Null | Notes |
|--------|------|------|-------|
| `loyalty_program_id` | uuid FK | no | transitional; meaning is the **Shop** of the membership (see [Shop capability model](#shop-capability-model-decided-not-shipped)) |
...
| `points_cost` | integer | no | amount reserved at create for this redemption. Whether a later catalog price change updates a `pending` row is **pending Product Owner** ([redemption §14 item 1](../product/reward-redemption-flow.md#14-pending-owner-decisions-do-not-implement-yet)) |
| `status` | text | no | catalog redeem Phase 1: `pending` \| `completed` \| `expired` \| `rejected`. Visit **earn** rows may still use `earned` until an explicit redeem. `rejected` is **not** a staff action on a valid physical QR (retained for other / non-QR invalidation). Optional later (not Phase 1): `cancelled` \| `reversed` |
...
Catalog redeem (physical): insert `pending`, **reserve** `points_cost`, set `qr_code` + `qr_expires_at`. Staff **scan** consumes the reservation (`UPDATE … WHERE status = 'pending'` and QR still valid; affected rows = 1) and sets `redeemed_at` + `completed`. Past-due `pending` rows are marked `expired` by a **scheduled job**, which releases Reserved — do not rely on client-side expiry. Insufficient Available at create is an error with **no row**. [reward-redemption-flow.md](../product/reward-redemption-flow.md).

### `referral_settings` — both-party kinds + expiry

Existing row (one per Shop): `enabled`, `referrer_bonus_points`, `new_customer_discount_pct`. Target additions:
...
| `customer_rewards` pending/completed/expired + reserved `points_cost` + `qr_code` / `qr_expires_at` | Catalog redeem reserve / QR verify / expiry job | G-20 |
...
Tier must update automatically whenever `customers.points` or `customers.visits` change (check-in, redeem, referral, adjustment, POS), not only when application code remembers to call a writer. Ladder edits must recompute the whole Shop. **Open:** whether the metric is those spendable counters or a lifetime cumulative figure that does not fall on redeem / stamp reset ([program-model.md](../product/program-model.md#open--tier-metric-source)). Until decided, the function below documents today’s intended writer (spendable counters) — do not treat that as a product lock.
...
10. **Shop loyalty capabilities (DECIDED membership, Shop QR resolved):** a Shop has at most one Points, one Visit, and one Tier capability. Target unique: **`UNIQUE (owner_id, program_type)`**. Drop `UNIQUE (owner_id)` (that blocks Points+Visit). Do **not** drop uniqueness entirely (that would allow two Points configs). Each capability may have `status` = `draft` \| `active` \| `disabled`. Join/check-in when **at least one** capability is `active`. One membership per Shop; first join creates that membership only; returning scan is check-in, no duplicate row. Conditional earning modifiers (e.g. Happy Hour 3x) are **rules inside Points** sharing one balance. How earning rules are stored is backend-owned. Shop QR always resolves to this Shop. Catalog, referrals, wallet, and redemptions are Shop-scoped. [program-model.md](../product/program-model.md) · [loyalty-page.md](../frontend/loyalty-page.md#shop-loyalty-capabilities-decided) · [counter QR](../product/counter-qr-and-program-membership.md).
...
12. **Referral grants (DECIDED, not shipped):** both parties get a reward. **OTP first:** do not INSERT `customers`, `referrals`, `points_ledger`, or `vouchers` until SMS/WhatsApp OTP is verified ([write rule 14](#binding-write-rules)). **Referred** (new): grant in the same transaction as OTP-verified **new** enroll/register with valid `?ref=` / code — never on returning check-in. **Referrer** (existing): grant **only** on the referred customer’s **first `Invoice.Paid`** (`orders.paid_at` set) in that program **and** `referrals.status = pending` (not `pending_review`). Unpaid order inserts do not grant. That paid-invoice gate is the primary economic anti-fraud control. Kind per side from `referral_settings`: **`points`** → `customers.points` + `points_ledger` (`reason = referral`, `expires_at` **required**) in the same transaction; **`discount`** → `vouchers` row (`status = active`, `expires_at` required) to redeem later — do **not** auto-apply a % on the join, cart, or first bill; do **not** write referral discounts to `customer_rewards`. Share is that member’s **link** or **QR** of this Shop’s join URL `?ref={referral_code}` on **that Shop’s customer wallet card**. **Signup Bonus** is separate: once per customer per Shop on first membership join, regardless of referral; it **can stack** with the referred-party grant when both are configured. Welcome / link-Shop screens do not grant unless Signup Bonus is configured. If both Points signup bonus and Visit signup stamp are configured, they **can both fire** on first Shop join. Ledger `reason = signup_bonus` for that grant. [Signup vs Referral](../frontend/loyalty-page.md#signup-bonus-vs-referral-bonus-decided). **Open:** `points` referral kind when Points capability is disabled. **DB + app:** `CHECK (referrer_id <> referred_id)` and `UNIQUE (referred_id)` — self-invite and a second attribution **fail the insert**. **Device/IP:** same device or same public IP in the same minute → `pending_review`; do not grant the referrer until review clears to `pending` (then grant on first **paid** invoice, or immediately if `paid_at` already exists). [loyalty-page.md](../frontend/loyalty-page.md#referral-rewards-decided) · [fraud controls](../frontend/loyalty-page.md#referral-fraud-controls-decided) · [OTP](../frontend/loyalty-page.md#otp-verification-decided).
13. **Shop-scoped point lots (DECIDED, not shipped):** issued lots always carry Shop identity (`loyalty_program_id` as transitional alias). Stamp `expires_at` per [expiry source](#points_ledger). `customers.points` is the **spendable / available** sum for **that** Shop membership only (unexpired lots minus **pending reserved**). `Available = Total − Reserved`. Never add Shop A + Shop B for a customer total. Customer wallet lists one card per Shop: Points / Visit / Tier as sections when enabled. Also expiry groups, vouchers (`vouchers` table), share link/QR. [program-model.md](../product/program-model.md#4-customer-membership-and-wallet) · [loyalty-page.md](../frontend/loyalty-page.md#customer-wallet-per-shop-decided).
...
15. **Catalog redemption lifecycle (DECIDED Phase 1, not shipped):** create checks Available; if `Available < cost` → error, no row. If valid: `pending` + reserve `points_cost` + single-use `qr_code` with `qr_expires_at` +10 min. Create is idempotent (same operation returns the same row; do not reserve or re-issue QR twice). Staff **scan** is one transaction (`UPDATE … WHERE status = 'pending'` and QR still valid; affected rows = 1 → `COMPLETED` + consume reserved). Already `completed` → error **“already redeemed”** (no second deduct). Past due / `expired` → error **“expired”**. A scheduled job marks `pending` past `qr_expires_at` as `expired` and releases Reserved — do not rely on client-side expiry. Combined pending cost cannot exceed available; concurrent Redeems check Available, not Total. Cross-Shop spend is forbidden; the row stays on the Shop even if the customer later uses another Shop. Reward eligibility / expiry is evaluated at create; later reward `expires_at` must not auto-invalidate a pending row; QR TTL is independent. Earn and redeem share one concurrency model. Idempotency key / unique business reference on create (and on earn — write rule 16). Digital catalog rewards may complete instantly without QR. **Not Phase 1:** refund / reverse. **Do not implement** staff Approve/Reject for physical catalog rewards (superseded). **Pending Product Owner:** price change / reward disable-delete / program disable / reserved-lot expiry while PENDING. [reward-redemption-flow.md](../product/reward-redemption-flow.md) · [api-contract](api-contract.md#catalog-redemption-lifecycle-decided-not-shipped).
16. **Earn idempotency (DECIDED, not shipped):** each earning event (check-in, POS purchase, `Invoice.Paid`) has an idempotency key or unique business reference. The same business event awards points once (retry → no-op). Different legitimate events both process. Enforce a unique constraint / business reference at the database. Concurrent earn and redeem must serialize to a consistent available balance (no negative balance, lost update, double deduct, or incorrect reservation); never trust a client-side total.
...
| **Capability status** | Loyalty capability lifecycle: `draft` (not live) \| `active` (live) \| `disabled` (off). A Shop has at most one Points, one Visit, one Tier. | `loyalty_programs.status` (not shipped; today one row per owner). [program-model.md](../product/program-model.md) · [loyalty-page.md](../frontend/loyalty-page.md#shop-loyalty-capabilities-decided) |
| **Shop** | Loyalty unit: one membership, one wallet, one catalog. Optional Points / Visit / Tier capabilities. | [program-model.md](../product/program-model.md#1-shop-vs-capability) |
| **Capability** | Optional Points, Visit, or Tier config of a Shop. Not an independent program. At most one per type. | [program-model.md](../product/program-model.md) |
...
| **Spendable / available points** | Unexpired lots for **one** Shop membership, minus **pending reserved**. Not a cross-Shop total | `points_ledger` + pending redemptions; customer wallet |
...
| **Catalog redemption** | Customer request for a program reward: `pending` (QR, 10-min TTL) → `completed` (staff scan) \| `expired` (job). `rejected` only for other / non-QR invalidation — not staff refusal of a valid QR | `customer_rewards` (catalog path) |
