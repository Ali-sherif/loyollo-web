...
**Available vs reserved (DECIDED):** pending catalog redemptions reserve `points_cost` in that program **at Redeem time** (not at staff-scan time). `Available = Total − Reserved`. Concurrent Redeems check Available, not Total. Do not spend a lot after `expires_at` (how reservation interacts with **lot** expiry is **pending Product Owner** — [redemption §14 item 14](../product/reward-redemption-flow.md#14-pending-owner-decisions-do-not-implement-yet)). QR / pending TTL is 10 minutes and is independent ([§6b](../product/reward-redemption-flow.md#6b-expiry-handling-scheduled-job)).
...
10. **Multiple loyalty programs (DECIDED membership, Shop QR pending):** a shop has many `loyalty_programs`, including **multiple of the same `program_type`** (no 1-per-type; do not unique `(shop, program_type)`). Each has `status` = `draft` \| `active` \| `disabled`. Drop `UNIQUE (owner_id)`. Join/check-in only when the program is `active`. Selecting one Program never auto-joins siblings. First join of a program creates that membership only; returning scan is check-in, no duplicate row. Conditional earning modifiers (e.g. Happy Hour 3x) are **rules inside one Program** sharing one balance — not a second Program. How earning rules are stored is backend-owned. **Pending Business Owner:** whether more than one program may be `active` at once, and therefore Shop QR (one-active → `/join/{programId}`; multiple-active → selection then `/join/{programId}`). Creating any number is already decided. Do not finalize Shop QR until that decision. [program-model.md](../product/program-model.md) · [loyalty-page.md](../frontend/loyalty-page.md#multiple-programs-and-status-decided) · [counter QR §15](../product/counter-qr-and-program-membership.md#15-shop-qr--multiple-active-programs-pending).
...
| **Program status** | Loyalty program lifecycle: `draft` (not live) \| `active` (live) \| `disabled` (off). A shop has **many** programs, including same type. | `loyalty_programs.status` (not shipped; today one row per owner). [program-model.md](../product/program-model.md) · [loyalty-page.md](../frontend/loyalty-page.md#multiple-programs-and-status-decided) |
| **Shop** | Container for Programs. No shop-wide points, membership, or wallet. | [program-model.md](../product/program-model.md#1-shop-vs-program) |
| **Program** | Independent loyalty unit: members, wallet, points/stamps/tier, rewards, signup, referrals. Balances never mix across Programs. | [program-model.md](../product/program-model.md) |
