...
| Customer portal (DECIDED, not in approved map) | Shop-customer register/login — **routes not locked**; not `/app` `admin` / `staff` auth | customer identity + stored activity + calculated KPIs | [G-33](gaps-and-solutions.md#g-33--shop-customers-have-no-registerlogin-kpis-rely-on-owner-typed-rows), Auth |
