...
| Customer portal (DECIDED, not in approved map) | Shop-customer register/login — **routes not locked**; not `/app` `admin` / `staff` auth. Wallet: **one card per program** (points + expiry + share) | customer identity + stored activity + calculated KPIs + per-program wallet | [G-33](gaps-and-solutions.md#g-33--shop-customers-have-no-registerlogin-kpis-rely-on-owner-typed-rows), [loyalty-page.md](loyalty-page.md#customer-wallet-per-program-decided), Auth |
