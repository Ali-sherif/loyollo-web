...
| Loyalty        | `/app/loyalty`, `/join/[programId]` (approved); Shop QR URL **pending BO item 15** — still **one** program after resolve (never auto-join all) | programs, rewards, QR settings, catalog redeem QR verify | Customers, notifications; [counter QR](../product/counter-qr-and-program-membership.md) · [redemption](../product/reward-redemption-flow.md) |
