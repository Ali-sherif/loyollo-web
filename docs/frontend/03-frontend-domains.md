...
| Loyalty        | `/app/loyalty`, `/join/[programId]` (approved); **intended** `/join/shop/[shopSlug]` — shop entry, then **one** program (never auto-join all) | programs, rewards, QR settings, catalog redeem review | Customers, notifications; [counter QR](../product/counter-qr-and-program-membership.md) · [redemption](../product/reward-redemption-flow.md) |
