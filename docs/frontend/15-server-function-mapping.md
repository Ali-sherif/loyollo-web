...
| `deleteMyAccount`          | Backend API (Server Action only if clear UI/session benefit)  | Delete current user via NestJS Auth API                 | Settings danger zone                                    | NestJS auth + service-role data cleanup | Prefer backend; Server Action only if clear UI/session benefit → `lib/server/accounts`                                                                                                                                   | Destructive; cookie CSRF = Origin/Host + `SameSite=Lax` ([ADR-017](../architecture/decisions/ADR-017-csrf-cookie-mutations.md)); Nest Bearer from Next | Critical: destructive action, CSRF/session validation |
...
- Cookie-authenticated mutations: Origin/Host allow-list + `SameSite=Lax` ([ADR-017](../architecture/decisions/ADR-017-csrf-cookie-mutations.md)). Server Actions use Next.js built-in Origin check; cookie BFF Route Handlers apply the same check explicitly. Nest mutations use Bearer from Next, not browser cookies.
