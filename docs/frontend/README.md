...
- Backend owns authz via **NestJS independent auth** (local JWT for `admin` · `staff` · `customer`); **no Supabase Auth** even during Frontend Migration ([ADR-005](../architecture/decisions/ADR-005-authentication.md) Option C). Next.js route protection + session-aware rendering; HTTP-only cookies where applicable. Cookie-authenticated mutations: Origin/Host + `SameSite=Lax` ([ADR-017](../architecture/decisions/ADR-017-csrf-cookie-mutations.md)). NestJS natively handles admin temp-passwords/resets and customer OTP. Locked roles: `admin` · `staff` (same permissions as `admin` for now) · `customer` ([11-authentication-migration.md](11-authentication-migration.md#locked-role-matrix)). Product scope: [phase-1-scope.md](../product/phase-1-scope.md).
