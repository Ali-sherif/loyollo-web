...
**Date:** 2026-08-15
**Audience:** UI/UX team
**Purpose:** List of missing screens + flow written as steps and diagrams. Send this file straight to design.
...
### 5.1 New registration from QR / join page

The `/join/[programId]` page exists (QR Experience branding is wired up). Missing: the OTP step before the member is created.

**Steps**

1. Scans the QR or opens `/join/{programId}` (possibly with a personal invite `?ref=`).
2. Sees the branding + form (phone / channel). If the program is `draft` or `disabled` → **no join** — design an empty state.
3. Chooses **SMS** or **WhatsApp**. **No password field.**
4. Requests a code → `POST /api/join/otp/request`.
5. If **429** (rate limit): a toast appears, the button is disabled, **no silent retry**. **UX-68**
6. Enters the code (allow paste and `autocomplete="one-time-code"`).
7. Correct code **before** a `customers` row, referral, or reward is created. Wrong/expired code → **no account created**.
8. Success → customer portal (or a join-success screen if there's no session yet — product hasn't locked the URL yet).
9. If `?ref=` is valid: the reward for the new party happens in the same transaction. A returning scan with the same `ref` = check-in, **not** another referral.

```mermaid
flowchart TD
  QR["Scan QR\n/join/programId"] --> Prog{Program active?}
  Prog -->|No| Empty["Empty state: program unavailable"]
  ...
  Valid -->|No/expired| NoRow["No account created"]
  Valid -->|Yes| NewOrOld{Phone already in program?}
  NewOrOld -->|New| Enroll["Enroll + referral if ref"]
  NewOrOld -->|Returning| CheckIn["Check-in — no new OTP\nand no second referral"]
  Enroll --> Portal["Customer portal / wallet"]
  CheckIn --> Portal
```

### 5.2 Customer login / lost access (same mechanism)

Product decision locked: login = new OTP on the same channel. No "forgot password" screen.

**Steps**

1. Opens the customer login screen (path proposed by design, not `/auth/sign-in`).
2. Phone + SMS or WhatsApp → requests a code.
3. Correct code → the portal. **Never** `/app` or `/auth/reset-password`.
4. Wrong/expired code → requests a new code (lost access = same flow).
5. `inactive` account → session forbidden. Design an "inactive account" state without revealing the account's existence in a way that helps an attacker (generic message). **UX-06**
6. Unknown phone number: empty state / redirect to registration or join — open for design + product.

```mermaid
flowchart TD
  Login["Customer login screen UX-06\nnot /auth/sign-in"] --> OTP["Request OTP"]
  OTP --> Code["Enter code"]
  Code --> Ok{Valid?}
  Ok -->|No| OTP
  Ok -->|Yes| Status{account_status}
  Status -->|inactive| Block["Forbidden state — generic message"]
  Status -->|active| Portal["Customer portal"]
  Lost["Lost access"] --> OTP
```

Is registration and login one screen or two? **Not locked.** Product treats them as the same OTP. Design decides, but don't add a password field in either version.
...
| Sharing           | Personal link + QR `/join/{programId}?ref={code}` — this is not the store's QR |
...
| Program not `active`               | No join                                                                      |
| `inactive` account                 | No session                                                                   |
| Double-click / retry after timeout | OTP is once-use — show a "code already used" state, not a raw database error |
...
2. Customer P0 flow: UX-05, UX-06, UX-08, UX-07, UX-09 + OTP copy (**UX-62**).
...
- Portal URL
- OTP TTL and attempt count (draw the state, not the number)
- Whether registration and login are one screen
