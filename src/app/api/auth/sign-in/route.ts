...
import { nestSignIn } from "@/lib/server/auth/nest-client";
...
export async function POST(request: Request) {
  ...
  const result = await nestSignIn(body.email, body.password);
  if (!result.ok || !result.data) {
    const message =
      (result.data as { message?: string } | null)?.message ?? "Invalid email or password.";
